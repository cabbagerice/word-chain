...
#print(db.reference("/PDD_hum").get());exit()
...
start = time.time()
print(db.reference("/PDD_hum/あ").get())
...
for query in dicdata:
	...
	head=list(query)[0]
	#print(head)
	num=list(query[head])[0]
	#print(query[head][num])
	json=db.reference("/PDD_hum").get()
	if head in list(json):
		#DBに頭文字がおる>単語がいない
		if not int(num) in [i for i in range(0,len(json[head])-1)]:
			appendData("/PDD_hum/"+head,num,query[head][num])
	#DBに頭文字がおらない
	else:
		appendData("/PDD_hum",head,query[head])
